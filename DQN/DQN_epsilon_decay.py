...
# 定义强化学习模型
class DQNAgent():
    def __init__(self, q_func, optimizer, n_act, reply_buffer, batch_size, explorer, update_target_steps=100,
...
# 训练模型
class TrainManager():
    def __init__(self, env, episode=1000, lr=0.001, gamma=0.9, e_greed=0.1, e_greed_decay=1e-6,
                 memory_size=2000,
                 reply_start_size=200,
                 batch_size=23):
        self.env = env
    ...
    def train(self):
        ...
        for epoch in range(self.episode):
            ...
            if epoch % 100 == 0:
                ...
                cnt += 1
                x2.append(cnt)
                y2.append(test_reward)
            if epoch % 200 == 0:
                print('e_greed=%.2f' % self.agent.explorer.epsilon)
            x1.append(epoch)
            y1.append(ep_reward)
        plt.figure()
        plt.plot(x1, y1)
